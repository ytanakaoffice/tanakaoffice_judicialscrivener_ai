...
# ページの設定
st.set_page_config(
    page_title="田中式 司法書士AI一問一答（美少女AI講師への24時間質問サポート付）", page_icon="📖", layout="centered"
)
...
def check_password():
    ...
    st.title("田中式 司法書士AI一問一答（美少女AI講師への24時間質問サポート付）")
    st.subheader("ログイン認証")
    password = st.text_input("パスワードを入力してください", type="password")
    
    # st.secretsからパスワードを取得
...
# ---------------------------------------------------------
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [{
            "role": "assistant",
            "content": (
                "こんにちは！田中式 司法書士AI一問一答へようこそ。美少女AI講師のAIたなかっち1号先生が24時間あなたの学習をサポートします。\n\n"
                "※現在は試験導入中のため、1日30ラリーまでとさせていただいております。"
                "今後人気が出次第、無制限に使えるようにアップデート予定です！応援よろしくお願いします。"
            ),
        }]

    for message in st.session_state.messages:
        avatar_img = "images/1_teacher_normal.png" if message["role"] == "assistant" else None
    ...
    if st.session_state.chat_count >= 30:
        ...
        prompt = st.chat_input("全体的な学習法や、特定の法律用語について入力してください...", key="main_chat_input_field")
        if prompt:
            st.session_state.chat_count += 1
    ...
    if st.session_state.get("main_waiting", False):
        st.session_state.main_waiting = False
        with st.chat_message("assistant", avatar="images/1_teacher_normal.png"):
            with st.spinner("AIたなかっち1号先生が考えています..."):
                last_prompt = st.session_state.messages[-1]["content"]
                response_text, new_conv_id = call_dify(last_prompt, st.session_state.dify_conversation_id)
                if new_conv_id:
